  ...
  return (
    <header className="sm:sticky top-0 z-50 bg-white p-2 md:p-3 border-b border-gray-200">
      <div className="max-w-6xl mx-auto w-full px-2 md:px-4 relative">
        {/* Mobile/Tablet Notification Bell - Top Right */}
        {isLoggedIn && (
          <div className="absolute top-2 right-2 z-10 lg:hidden notification-container">
            <button
              onClick={toggleNotifications}
              className="relative p-2 text-gray-600 hover:text-orange-500 transition-colors duration-200 focus:outline-none bg-white rounded-full shadow-sm border"
              aria-label="Notifications"
            >
              <FaBell className="text-lg" />
              {unreadCount > 0 && (
                <span className={`absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center font-bold transition-all duration-300 ${countChanged ? 'animate-bounce scale-110' : ''}`}>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-3 md:gap-6 lg:gap-8 items-center justify-between min-h-[70px]">
          ...
          <div className="flex items-center w-full sm:w-auto gap-3 sm:gap-4">
            ...
            {!isLoggedIn && (
              <div className="sm:hidden ml-auto mt-1">
                <button onClick={() => navigate("/login")} className="flex items-center bg-orange-500 text-white text-xs rounded-sm p-1.5 hover:underline">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-2 lg:gap-5 w-full sm:w-auto flex-1 min-w-0">
            ...
            {isLoggedIn && (
              <div className="relative ml-2 md:ml-4 notification-container hidden lg:block">
                <button
                  onClick={toggleNotifications}
